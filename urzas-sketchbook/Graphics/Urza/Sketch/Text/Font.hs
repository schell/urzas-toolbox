module Graphics.Urza.Sketch.Text.Font (
...
import           Graphics.Urza.Sketch.LoadTexture
...
texturizeGlyphOfEnum file px enm = do
    ...
    runFreeType ("Setting pixel size to " ++ show px) $
        ft_Set_Pixel_Sizes ff 0 $ fromIntegral px
    ...
    runFreeType ("Loading glyph " ++ [(toEnum . fromEnum) enm]) $ ft_Load_Glyph ff ndx 0
    -- Get the GlyphSlot.
    slot <- peek $ glyph ff
    -- Get the char bitmap.
    runFreeType ("Rendering glyph " ++ [(toEnum . fromEnum) enm]) $ ft_Render_Glyph slot ft_RENDER_MODE_NORMAL
    ...
    -- Clean up.
    runFreeType "Cleaning up." $ ft_Done_FreeType ft
...
runFreeType :: String -> IO FT_Error -> IO ()
runFreeType s m = do r <- m
                     unless (r == 0) $
                         fail $ "freetype2 error " ++ show r ++ " while:\n    " ++ s


-- | Retrieves a marshaled FT_Library to use in freetype calls.
freeType :: IO FT_Library
freeType = alloca $ \p -> do runFreeType "Getting freetype2 library reference." $ ft_Init_FreeType p
                             peek p
...
fontFace ft fp = withCString fp $ \str ->
    alloca $ \ptr -> do runFreeType ("Loading font face '" ++ fp ++ "'") $ ft_New_Face ft str 0 ptr
                        peek ptr
